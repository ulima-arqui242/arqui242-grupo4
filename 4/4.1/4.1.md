...
| **Atributo de Calidad** | **Estímulo**                              | **Fuente del Estímulo**        | **Artefacto**                    | **Entorno**         | **Respuesta**                                                                         | **Medida de Respuesta**                                  |
|-------------------------|-------------------------------------------|--------------------------------|----------------------------------|--------------------|---------------------------------------------------------------------------------------|----------------------------------------------------------|
| Disponibilidad           | servicio de pago externo deja de estar disponible   | Proveedor del servicios | Módulo de pago | Operación en tiempo real | El sistema cambia automáticamente a un método de pago alternativo | transacción se redirige en menos de 2 segundos
 |

## Táctica elegida
Se decide implementar un mecanismo de reintento para llamadas fallidas a las APIs externas, lo que permitirá cambiar entre diferentes pasarelas de pago en caso de que alguna no esté temporalmente disponible.

## Documentación de la Decisión (ADR)

### Título:

implememtacion de failover en el metodo de pago cuando el servidor externo no esta disponible

### Contexto:

El módulo de pagos permite realizar pagos utilizando servicios externos. Si los servicios externosfallan o se interrumpen, el sistema de pagos pueda redirigir la transacción de manera automática a otro método de pago disponible para la continuidad del servicio

### Alternativas:

### prediccion:

- monitoree el estado de los servicios de pago externos
- redirija los pagos automáticamente a los métodos disponibles.

### excepciones:

- manejo de excepciones para capturar errores y fallas en los servicios externos de pago
- el sistema intentaría nuevamente la operación utilizando un método alternativo
...
La alternativa que se escogió fue Excepciones

### Sustento:
garantizar alta disponibilidad y una respuesta rápida, sin sobrecargar el sistema con predicciones o monitoreos innecesarios. asi se asegura que los usuarios no experimenten interrupciones en sus transacciones, mientras que el sistema reacciona de manera automática ante cualquier falla en los servicios de pago.